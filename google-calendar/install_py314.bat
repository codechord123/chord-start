@echo off
title Teacher Calendar - Install (Python 3.14)
echo ============================================
echo  Teacher Calendar - Install for Python 3.14
echo ============================================
echo.
...
for /f "tokens=*" %%v in ('py -3.14 --version 2^>^&1') do echo Python: %%v
echo.

echo [1/2] Installing PyQt6 and PyQt6-WebEngine for Python 3.14...
echo       (First run downloads about 150MB, please wait)
py -3.14 -m pip install --upgrade PyQt6 PyQt6-WebEngine
if errorlevel 1 (
    echo.
...
echo [2/2] Adding to Windows startup (Python 3.14)...
set "VBS=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup\TeacherCalendar.vbs"
echo CreateObject("WScript.Shell").Run "pyw -3.14 ""%~dp0desktop_app.py""", 0, False> "%VBS%"
echo   Registered: %VBS%
echo.
...
echo ============================================
echo.
start "" pyw -3.14 "%~dp0desktop_app.py"
